...
- W4 uses a portable ZIP with a Chinese double-click entry backed by a PowerShell registration script, not Setup.exe/MSI. The archive may be extracted anywhere; registration writes `WORKBUDDY-RUNTIME.json` beside each installed Skill so WorkBuddy can find the stable launcher and actual data root, then records a post-install offline `doctor` result.
- Repository `setup.py` is consumer-owned development metadata and is deliberately excluded from the end-user ZIP.
