...
Stage 4 planning is `PASS_ACCEPTED`: the reviewed plan was promoted at `5cb3f585a0cddffbd823c785b1d39ebd1834c1df`, and its state closeout was promoted at `dfd97f3d2e05a4c448448fc14514d1cfe76836e8`. The user's 2026-08-20 instruction satisfies the separate explicit-start condition; `V2-S4-IMPLEMENTATION-AUTHORIZATION-REVIEW1` returned `APPROVE / P0=0 / P1=0 / P2=0`, authorization result `2c3d87bedfa4a3cef3cfd952641199300f2715dc` was ordinary-fast-forwarded, and CI run `32340096961` completed successfully. The remaining governance action is independent zero-write `V2-S4-IMPLEMENTATION-AUTHORITY-SYNC-REVIEW1` review and ordinary fast-forward of the six-authority sync candidate; only after both may `V2-S4-IMPLEMENTATION-BUILDER1` take over from the then-latest formal exact commit/tree. Before that sync completes, no production edit is authorized. The accepted minimum contract is provider- and runtime-opaque: a release-specific immutable `PackageToolDefinitionV1` supplied by the approved Package definition/final-delivery Installer owner, one public `launch_session_tool(data_root, user_message, executor_controls, package_tool_definition, local_capability_evidence=(), cancel_event=None)` entry, exactly one fixed Package-tool spawn, and one recursively immutable `LauncherReceiptV1` whose outcome is limited to the nine accepted values. Stage 4 first calls `locate_active_package(data_root)` and revalidates the Package, required toolchain, definition, tool, and interpreter; it never guesses an entry from a Guide, directory, registry, caller command, or system PATH.
