...
Stage 3 owns only optional capability preparation requested by a verified WorkBuddy/OpenMontage production decision. The allowed catalog is currently Remotion and HyperFrames, but each immutable Package Release may declare support for zero, one, or both; neither runtime is a required bundled prerequisite. A Package-owned capability Lock is required only for a capability that the exact Release declares supported, and Stage 3 touches only the one capability selected for the current request plus its Lock-declared browser/runtime assets. Shell does not select the renderer, and ordinary users approve the locked download plan rather than choosing technical components. Optional discovery may use only managed paths, an explicitly registered capability record, or normal command resolution; never scan drives. Mainland-China mirrors are mandatory for end-user optional downloads and no automatic overseas fallback is allowed.

Stages 3, 4, 5, and 6 are built and accepted in numeric order, but end-user use starts at the Stage 5 WorkBuddy entry. Stage 5 asks Stage 2 Locator to revalidate the retained production Package. Stage 4 may make a base fixed-tool call using only that verified required toolchain. WorkBuddy/OpenMontage then owns the renderer decision. A bundled-FFmpeg path continues without Stage 3; execution of a Package-declared optional Remotion or HyperFrames capability additionally requires a Stage 3 receipt bound to the same Registration and that capability's Package-owned Lock. An undeclared optional capability is `BLOCKED`, not an installation target. WorkBuddy owns pause, consent, and continuation; Shell never selects the renderer or automatically replays the original business request. Stage 6 only relays the resulting facts.

The previous Stage 3 execution packet that treated Python dependencies, FFmpeg, and Node as discover/install targets is superseded. Replanned Stage 3 has one proposed public entry, `prepare_optional_capability(data_root, capability_request, authorization_receipt=None)`, and at most one new production module plus one export-only edit and one direct test file. Its Lock is owned by the verified Package, not duplicated by Shell. Before Stage 3 implementation, freeze only the minimal producer/consumer interface: the five result states and WorkBuddy actions for continue, reuse, consent, prepare, or block. Real WorkBuddy new-session and same-task-continuation evidence belongs to Stage 5 acceptance and must not circularly block Stage 3. Stage 3 implementation remains `NOT_GRANTED` until a final Package is retained and production-registered, Locator revalidation succeeds in a new process, every optional capability declared by that exact Release has a Manifest-covered Package-owned Lock, the minimal interface contract is frozen, and the live task authority grants the exact Builder packet. A Release that declares zero optional capabilities may proceed through final-Package delivery, but provides no positive Stage 3 implementation input; Stage 3 must remain zero-code/not-required for that Release rather than manufacture a generic framework.
