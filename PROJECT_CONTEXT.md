...
Stage 3 implementation and Stages 4-6 are not authorized. They are built and accepted in numeric order, but actual use starts at the Stage 5 WorkBuddy entry, then revalidates the Stage 2 Locator, obtains one Stage 3 closed-set runtime outcome, calls Stage 4 only with a valid runtime-ready receipt, and lets Stage 6 relay facts. A missing/incompatible outcome is relayed as one missing-only plan; after separate explicit consent Stage 3 prepares the confirmed items and stops, with no automatic retry of the original request. Private Python is bundled in the Golden Key Package; Stage 3 covers Python dependencies, FFmpeg, Node, Remotion, HyperFrames, and the locked browser. End-user sources are approved mainland-China mirrors plus one temporary exact FFmpeg `gyan.dev` exception gated by a no-proxy/no-VPN mainland-network probe. No Launcher, WorkBuddy entry, runtime preparation, or status/result relay implementation may be inferred from documentation authorization.

The Stage 3 execution packet is frozen but conditionally gated. After the refreshed Stage 2 and this planning correction are independently promoted, a read-only handoff audit must validate exact Package identity, the executable private-Python bootstrap, complete inputs for a current Runtime Lock, the formal Git object, and the FFmpeg access state. Only then may the live authority switch Stage 3 from `NOT_GRANTED` to `GRANTED`. The planned implementation is exactly one `runtime_prepare.py` public entry, one production Runtime Lock, and one direct test file, with product-owned writes confined to `<DataRoot>/Runtime` and `<DataRoot>/Caches`.
