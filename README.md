...
The Stage 3 task packet is frozen under conditional authorization, not active implementation permission. It may start only after the refreshed Stage 2 and this planning correction are independently promoted and a read-only handoff audit passes. Its maximum implementation is one `runtime_prepare.py` public entry, one production Runtime Lock, and one direct test file; managed writes are confined to `<DataRoot>/Runtime` and `<DataRoot>/Caches`.
