...
厂商直连，以及当前经第三方网关接入的 Seedance、MiniMax 等能力。引导先让用户选择“生成图片、生成视频、
中文配音、数字人或内容分析”等实际目标，再只标出相关的一到两个推荐接入；其他Provider保留为高级选择，
不要求新手先理解环境变量，也不要求一次配齐所有服务。

每个推荐项会说明厂商直连或第三方网关、能解决的能力、当前状态、所需凭据的中文名称、官方申请/管理入口、
账户与地区限制以及费用提醒。用户明确选择“现在配置”后，WorkBuddy可打开可见的本地配置窗口；若宿主不能
直接打开，用户仍可双击`配置API密钥.cmd`。

用户不需要把API Key发送到WorkBuddy聊天。Key在本地窗口中隐藏录入，使用Windows当前用户DPAPI加密后保存在
已登记的数据目录；启动器只在当前调用进程中解密注入。`已录入但未验证`只说明Key存在，不代表账号权限、
余额、地区、网络或具体模型可用，也不构成一次真实或付费Provider调用授权。
