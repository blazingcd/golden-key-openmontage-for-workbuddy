...
The previous Stage 3 task packet, public-entry signature, and Shell-owned all-component Runtime Lock are `SUPERSEDED`. Stage 3 remains `NOT_GRANTED` until the final Release is retained, installed and production-registered, Locator succeeds in a new process, each capability declared by that Release has a Manifest-covered Package-owned Lock, the minimal result-to-WorkBuddy-action interface is frozen, and an exact Builder packet is explicitly granted. Real WorkBuddy new-session and continuation proof is Stage 5 acceptance, not a Stage 3 prerequisite.
