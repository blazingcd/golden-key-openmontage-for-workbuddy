...
- 在正式升级合同完成前，安装记录版本与当前包版本不一致时拒绝替换；旧包不能静默降级新安装，新包也不能绕过升级步骤。
...
当前切片解决“无卸载直接覆盖解压/删目录再装”的常见行为，但不等于跨版本升级/降级、完整卸载和事务级版本回滚已经验收。

## 尚未通过

- 全新普通Windows环境的完整Python依赖准备、跨版本升级/降级、卸载和回滚；
