...
M1.3 execution-contract freeze; it authorizes no Skill/Shell/Package/test change,
...
| M1.3 — selected local and API-key configuration | Start from ordinary language, choose one relevant optional capability, understand value/impact, approve or defer safely, and later recheck without operating commands or exposing a key in chat. | Before implementation, prove a private structured action carrier and a secure non-chat input route. Local evidence: one consent-bound managed Remotion or HyperFrames install, rediscovery, Package recognition, and actual Package-mediated invocation. API evidence: one formally declared Provider, secure local storage, exact allowlisting, secret absent from all observable transport, and an authorized Package-owned connection test. Both paths preserve FFmpeg, honest failure/defer, and later natural-language retest; no automatic substitution, Provider ranking by Shell, media call, fixed transcript, or second control plane. | `EXECUTION_CONTRACT_FROZEN / IMPLEMENTATION_NOT_STARTED / ACTION_TIME_GATES_REQUIRED` |
