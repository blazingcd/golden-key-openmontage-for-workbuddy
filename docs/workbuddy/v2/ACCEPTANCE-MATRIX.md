...
|---|---|---|---|
| M1 — first-use guidance and capability readiness | Understand that FFmpeg-ready basic production is available, see relevant optional enhancements, choose to continue or configure, and later inspect/change/retest through natural language. For mainland external video, understand Seedance as the current default recommendation and Kling, Vidu, and MiniMax as core alternatives without being told an unintegrated option is ready. | Verified-PackageRoot inventory; honest Remotion, HyperFrames, external video generation, and TTS states; Seedance/Kling/Vidu/MiniMax status distinctions; Vidu remains unavailable until its adapter gap is closed; non-blocking optional absence; WorkBuddy-owned Provider/model, credentials, privacy, cost, consent, test, and recovery; one representative selected configuration path; no secret relay, Shell decision, fixed grammar, false ready state, or user-visible internal mechanics. | `M1.1_FACT_AUDIT_COMPLETE / OWNER_ACCEPTED / ZERO_WRITE_DEVIATION_RECORDED / PROVIDER_SCOPE_DOCUMENTED / M1.2_NOT_AUTHORIZED` |
...
| C1 — broad Provider/model coverage beyond the initial set | Expand beyond Seedance, Kling, Vidu, and MiniMax only after repeated unmet demand proves the need. | Separate cost/risk plan and evidence of repeated unmet user demand; C1 cannot defer the four M1 Provider families. | `DEFERRED` |
...
The four initial mainland video Provider families are a product-scope decision,
not proof of live availability. Static Package inspection currently supports
Seedance, Kling, and MiniMax routes and identifies Vidu as an unclosed adapter
dependency. Any live configuration or connection evidence requires separate
