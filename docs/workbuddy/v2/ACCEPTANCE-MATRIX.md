...
| M1.3 — selected local and API-key configuration | Start from ordinary language, choose one relevant optional capability, understand value/impact, approve or defer safely, and later recheck without operating commands or exposing a key in chat. | The same-Skill action, WinCred wrapper, secret-safe transport, and Package `/ping` dispatch now have local code/test evidence only. Local evidence uses system-wide scope by default; current-user scope requires an explicit informed choice. Windows resolves the standard location without a fixed drive, then the complete locked npm closure comes from `registry.npmmirror.com`, followed by atomic installation, recorded-path handoff, rediscovery, Package recognition, and actual Package-mediated invocation. API evidence still requires real secure input/storage and an authorized live non-media connection result. Both paths preserve FFmpeg, honest failure/defer, and later natural-language retest; no silent scope/drive/source change, automatic substitution, Provider ranking by Shell, media call, fixed transcript, or second control plane. | `IMPLEMENTATION_LOCAL_CONTRACT_COMMITTED / LOCAL_ROUTE_REQUIRES_NPM_AND_PATH_CORRECTION / API_ROUTE_REAL_AUTH_NOT_PROVED / PRODUCT_INCOMPLETE` |
...
time. Shell records and supplies that exact path; Package consumes it. A different
