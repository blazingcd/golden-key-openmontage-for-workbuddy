...
| M1.3 — selected local and API-key configuration | Start from ordinary language, choose one relevant optional capability, understand value/impact, approve or defer safely, and later recheck without operating commands or exposing a key in chat. | The same-Skill action, WinCred wrapper, secret-safe transport, and Package `/ping` dispatch now have local code/test evidence only. Local evidence still requires non-empty verified definitions, one consent-bound managed Remotion or HyperFrames install, rediscovery, Package recognition, and actual Package-mediated invocation. API evidence still requires real secure input/storage and an authorized live non-media connection result. Both paths preserve FFmpeg, honest failure/defer, and later natural-language retest; no automatic substitution, Provider ranking by Shell, media call, fixed transcript, or second control plane. | `IMPLEMENTATION_LOCAL_CONTRACT_COMMITTED / LOCAL_ROUTE_BLOCKED_EMPTY_DEFINITIONS / API_ROUTE_REAL_AUTH_NOT_PROVED / PRODUCT_INCOMPLETE` |
