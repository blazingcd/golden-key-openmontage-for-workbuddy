...
The Owner's 2026-09-05 correction supersedes every older nested-release statement:
the user extracts one distribution ZIP once to the chosen final location, then
runs its top-level `安装到WorkBuddy.cmd`; the CMD validates and registers the
adjacent complete PackageRoot in place, with no inner ZIP extraction, no copy to
`%ProgramFiles%`, and no internal-path decision. Skill lifecycle remains a separate
official-UI-assisted product block until a supported WorkBuddy interface exists.
...
| Installer / Release Assembly | Extract one formal ZIP once to the desired final location, then start its top-level `安装到WorkBuddy.cmd` without operating internal paths or receiving a machine-bound test artifact. | ZIP contains the CMD beside the complete PackageRoot; CMD validates and registers that PackageRoot in place without inner extraction, `%ProgramFiles%` copy, or UAC-only install dependency; existing Registration objects remain readable; prior active Package/Registration/user data are retained. Skill lifecycle still requires a supported WorkBuddy interface or explicit official-UI evidence and cannot be called complete from Package registration alone. | `NESTED_RELEASE_FLOW_REJECTED_BY_OWNER / IMPLEMENTATION_PUSHED_32EE9E0 / FOCUSED_170_PASS_1_SKIP / REVIEW_APPROVE_P0_0_P1_0_P2_0 / SINGLE_EXTRACTION_ZIP_SHA256_3B628B61 / LOCALAPPDATA_RESIDUE_REMOVED / PROGRAMFILES_CLEANUP_CANCELLED_BY_USER / PACKAGE_FLOW_VERIFIED / SKILL_LIFECYCLE_BLOCKED_EXTERNAL` |
