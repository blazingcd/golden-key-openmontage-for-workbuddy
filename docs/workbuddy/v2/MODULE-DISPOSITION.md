...
| `runtime_prepare.py`、`host_tools.py`、Runtime locks、`subprocess_guard/**` | doctor、CLI runtime、安装repair、宿主工具发现、missing-only准备、运行时/离线测试 | Runtime按需准备 / `SELECTIVE_ADAPT` | 迁移闭集发现与missing-only合同：包内Python固定；其私有依赖、FFmpeg、Node、Remotion、HyperFrames和锁定浏览器按`managed`、`registered_host`、`PATH_host`、`missing`分类；用户确认后只准备锁定缺失项；大陆镜像、hash、staging、所有权和幂等强制执行 | 恢复旧大型文件；扫盘；扫描或替换包内Python；通用下载/包管理/公共resume/repair；未授权下载；未批准海外源回退；Shell选择渲染引擎/版本；修改系统Python/PATH | 活动Package Registration和Runtime Lock声明闭集需求，未来Launcher直接消费组件就绪事实 | discover/plan零写入；有效宿主工具可登记复用；只准备确认计划中的缺失项；镜像/hash/许可/目标不全fail closed；二次调用零下载复用 |
