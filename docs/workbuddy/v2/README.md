...
本仓库只实现WorkBuddy Shell V2。腾讯WorkBuddy是唯一运行中的Agent；它读取已验证金钥匙版OpenMontage执行包后承担生产角色，不存在第二个OpenMontage Agent进程。Shell只负责六模块。当前正式树tracked精确37；Stage 3与Stage 4规划、实现均已`PASS_ACCEPTED`。Stage 5、Stage 6、最终Package物化和生产登记仍为`NOT_GRANTED`或未证明。
