...
STAGE_4_PLANNING_ELIGIBLE_AFTER_CURRENT_DOCS_PROMOTION
STAGE_4_IMPLEMENTATION_NOT_GRANTED
STAGE_5_WORKBUDDY_ENTRY_NOT_GRANTED
STAGE_6_STATUS_RESULT_RELAY_NOT_GRANTED
```

本仓库只实现WorkBuddy Shell V2。腾讯WorkBuddy是唯一运行中的Agent；它读取已验证金钥匙版OpenMontage执行包后承担生产角色，不存在第二个OpenMontage Agent进程。Shell只负责六模块。当前正式树tracked精确35；Stage 3已完成并正式收口，本次只同步Stage4接管文档，不实施任何后续模块。
...
旧阶段3可执行任务包、入口签名、Shell自有全组件Runtime Lock和条件授权已经`SUPERSEDED`。阶段5拥有真实WorkBuddy新会话、唯一入口、literal `user_message`不变、逐能力询问和同任务继续；这些不是Stage4前置。阶段6只在Stage4回执和Stage5真实消费者存在后判断，可直接消费时以`STAGE_6_DIRECT_LAUNCHER_RECEIPT_REUSE`和生产代码0结束。

本docs sync推广后只具备启动Stage4规划/接管判断的条件，Stage4实现仍为`NOT_GRANTED`且`next_authorized_task=NONE`。Stage4先调用`locate_active_package(data_root)`，基础固定工具调用不依赖Remotion/HyperFrames；明确执行可选能力时才要求同一capability+definition的Stage3 `PRESENT`或`INTEGRATED`证据。当前Locator没有固定Package工具入口身份，Stage4精确公共入口和不可改写回执字段也未冻结，必须由后续单独授权的Stage4规划任务闭合。最终Package/生产Registration归后续最终交付或Installer任务，最迟在Stage5真实生产验收前完成，不是Stage4规划/编码前置。
