...
`32ee9e03bae2af063d30d25781d8ff37621e09b6` is pushed. Focused checks pass
`170 passed, 1 skipped`; independent review is `APPROVE / P0=0 / P1=0 / P2=0`.
The verified single-extraction artifact is
`D:\BlazingCD\Personal\GoldenKeyData\WorkBuddyShellV2\installer-single-extract-20260905\golden-key-openmontage-for-workbuddy-0.3.25-single-extract-installer.zip`,
178,183,303 bytes, SHA256
`3b628b61af50a075af92cad8b74a9fbb36d11380de2aeaab99ed25c3f22aecbb`.
CRC, 8,782-member closure, top-level layout, CRLF CMD, old-flow absence, and
materialized Registration validation passed. No WorkBuddy or Skill action ran;
status is `PACKAGE_FLOW_VERIFIED / SKILL_LIFECYCLE_BLOCKED_EXTERNAL`.
...
WorkBuddy actually called `locate_active_package` and completed reading the exact
...
Remotion path and one representative API-key path. This authority does not expand
Installer into optional capability work, Provider/media action, S4 cross-machine
qualification, a second installer, private WorkBuddy storage mutation, or another
...
path, steps, wording, and intermediate conclusions may vary. Do not judge it
against an evaluator-invented creative or production transcript. Configuration
may use a bounded ordered sequence when actual WorkBuddy behavior, consent, secure
...
do not prove credentials, account permission, balance, connectivity, region,
price, or current model availability.

M1 plans two configuration mechanisms. Local capabilities such as Remotion and
HyperFrames require approved download, managed installation, rediscovery, Package
recognition, and actual Package-mediated invocation. API-key Providers require
secure input/storage outside ordinary chat, exact Package allowlisting,
non-disclosing relay, and an authorized connection test. Natural language owns the
user intent and consent; actual WorkBuddy evidence may change planned wording,
tools, or ordering without failing the task.
...
both representative M1.3 mechanisms are proved. M3 starts only after both M1.4
and M2 are complete.
...
| C1 — broad Provider/model coverage beyond formal Package declarations | Could | Add unsupported Providers or broader model, region, price-tier, and installation combinations after demand is known. | No speculative full matrix, blanket installation, or invented M1 entry. | Requires repeated unmet user demand and a separate cost/risk plan. Depends on successful selected paths under M1.3/S5. |
...
### Installer / Release Assembly — PACKAGE FLOW VERIFIED / SKILL LIFECYCLE BLOCKED
...
and does not use the current Installer's actual path binding. The v8 Skill contains
...
**Exact write set.** The implementation changes only:
...
4. Open the ordinary user-visible folder with the new ZIP selected. For a clean
   install, import it through WorkBuddy. For an upgrade, remove the same-name old
   Skill in WorkBuddy and import the new ZIP. Do not ask the user to type internal
   paths or commands, and do not read/write WorkBuddy private Skill storage.
5. Activate the new Registration only after the WorkBuddy UI import succeeds.
   Re-run Locator identity checks for PackageRoot, Guide, Manifest, Lock, and
   packaged toolchain.
6. Retain the prior Package/Registration and recovery ZIP. Remove only exact
   task-owned temporary paths after final review; never remove user data.

**Failure states.** Before old-Skill removal, every failure leaves the old
installation unchanged. A new-Skill import failure leaves the old Package active
and requires the recovery ZIP to be imported in WorkBuddy. If the new Skill was
imported but activation fails, the old pointer remains active and the recovery
Skill must be restored through WorkBuddy UI. A post-activation Locator failure
automatically restores the old pointer, while Skill restoration still occurs
through WorkBuddy UI. Report these separately as `PACKAGE_ROLLBACK_COMPLETE`,
`WORKBUDDY_SKILL_RESTORE_REQUIRED`, or `UI_ASSISTED_ROLLBACK_COMPLETE`; never
claim `AUTOMATIC_FULL_ROLLBACK`.

**Non-goals and anti-inflation.** Do not copy the old PowerShell/two-Skill flow,
add a second installer, embed a developer path, preinstall the v8 test ZIP, install
optional capabilities, handle credentials, call Providers, make media, build an
auto-updater, or absorb the S4 cross-machine maintenance matrix. The minimum clean
...
rollback case prove that exactly one correctly bound Skill remains and
`locate_active_package` resolves the installed Package and exact Guide. Result
labels are `VERIFIED_SINGLE_MACHINE_UI_ASSISTED_INSTALL` and, when exercised,
`VERIFIED_SINGLE_MACHINE_UI_ASSISTED_UPGRADE`; these are not S4 cross-machine
qualification. The result records immutable identities, receives one independent
zero-write review, and stops before Remotion/API-key/media work. M1.3 uses this
exact accepted release; M1.4 cannot repair missing Installer or M1.3 evidence.
...
- Static Package inspection found direct Seedance, Kling, and MiniMax routes.
  Current WorkBuddy tool inventory, actual
...
- The verified Package Guide historically described `provider_menu_summary()` as
  compact and said to deepen only when necessary, while the Package
  ...
  or mutate the Package. Later evidence proves that this raw summary is not a
  compact first-use display surface.
...
### Historical M1.2 corrected execution contract — WITHDRAWN / SUPERSEDED
...
At that historical checkpoint, M1.2's proposed product result was:
...
The historical corrected contract selected the smallest existing candidate first:
...
were reused; no Shell fact relay was built speculatively. This sequence is
retained as history and is not current execution authority; the product-flow reset
later in this register defines the current result and gate.
...
### Historical M1.2 probe result and Shell factual-relay audit gate — COMPLETE
...
- missing production path: no Shell caller; `user_entry.py` sends empty local
  ...
  existing handoff contains no Package factual summary;
...
`fixed_child.py` may read the raw Package summary only after the existing verified
...
### Historical M1.2 premature hard-stop correction and Skill consumer preparation checkpoint

At that preparation checkpoint, this direction superseded every unspent WorkBuddy
install/probe permission in the historical authorization immediately above.
...
At preparation start, planning local/tracking/advertised were
...
The Owner then authorized preparation through the next WorkBuddy action gate:
...
At that preparation checkpoint, the installed Skill remained the single Golden
Key Skill and was required to stay byte-identical at `SKILL.md` SHA256
...
`4112` bytes. It contains exactly `SKILL.md` and `scripts/run.ps1`; their SHA256
values are `20ab048d8e9cb9edee19f71a8fde81991670d4a571465c558ef1c02d8dd871ff`
and `0c8c743404127dbc612bc87c207f307ddbff2ef9a3a071b37c7ff2095383902b`.
It binds active-pointer SHA256
`807acbfd57254f6f1713bb99a9b8a9723c9e5e7010cfbac4eb8b6d93b8de96db`
and the already active final PackageRoot. The independent zero-write candidate
review is `APPROVE / P0=0 / P1=0 / P2=0`; the sole Skill source commit is
`c8eeb91e221ec96a406543c183091eea7ea6ac3c`. At that checkpoint, the installed
Skill remained unchanged and the WorkBuddy gate remained closed.

### Historical M1.2 factual-relay user result and consumer correction — EXECUTED

After the Owner manually uninstalled the old same-name Skill, the reviewed
handoff-consumer archive was installed as the only `golden-key-openmontage`.
One WorkBuddy 5.3.14 / `Hy3 0.00x` readiness probe used the ordinary request
`用金钥匙智能体看看我现在可以做哪些视频，还需要配置什么。`. The resulting
LauncherReceipt is `EXITED_SUCCESS` with a valid result pointer, one spawn, no
retry, the active final PackageRoot, and `fixed_child.py` SHA256
`66defdd34ea984b4b2ccf6d79753f90bf1c45f4b387f226552035c4e2ae136bf`.
The managed handoff SHA256 is
`da3196b7536a54453c27bea67fd3c339eb6ad4f4ab855836908d7ffb6706365c`.
Its ownership flags remain WorkBuddy-owned with no Provider/renderer selection,
production decision, or media execution. The task workspace
`C:\Users\blazi\WorkBuddy\2026-08-29-20-41-51` was empty after WorkBuddy removed
its temporary read-only parsing scripts.

This proves `WORKBUDDY_FACT_RELAY_REACHED_DIALOGUE`, but the independent result
review is `REJECT / P0=0 / P1=3 / P2=0`:

1. WorkBuddy called Remotion ready even though the active runtime facts reported
   `composition_runtimes.remotion=false`.
2. It exposed environment-variable mechanics to the ordinary user.
3. It did not clearly provide separate local configuration and defer choices.

Read-only fact inspection resolves the Remotion ambiguity. OpenMontage formally
integrates Remotion composer source, adapters, and a locked `4.0.484` dependency
set. The active PackageRoot contains those files but not
`remotion-composer/node_modules`, and no Package-mediated Remotion render has been
proved. Therefore the honest state is "integrated, dependencies not installed,
runtime not ready, invocation not verified". The generic subtitle rollup can
list provider `remotion` because that tool also declares FFmpeg fallback; it must
not override the composition runtime check.

The Owner authorizes the following bounded correction and push sequence:

1. Synchronize only the six authority/state documents, obtain one independent
   zero-write document review, commit, ordinarily push, and verify exact refs.
2. Ordinarily merge that exact planning head into the retained clean M1 branch.
3. Change only `workbuddy-skill/golden-key-openmontage/SKILL.md` so WorkBuddy
   separates integration from runtime readiness, does not expose raw environment
   names/paths/commands/URLs/install instructions, and always presents continue,
   local configuration, API-key configuration, and defer as four semantic choices.
4. Build one newly named uninstalled candidate bound to the unchanged active
...
uninstalled archive is
`D:\BlazingCD\Personal\GoldenKeyData\WorkBuddyShellV2\data\production\Integrations\WorkBuddy\golden-key-openmontage-0.3.25-m1.2-user-safe-readiness-3f4c75a.zip`,
size `4375` bytes, SHA256
`116737071d377b67dff6ea93fe18534114c17e762d32116efc5c4b973e000228`.
It contains exactly `SKILL.md` SHA256
`7d285376b2f3531c1891b2ebf40d6d612b9b7531be6776fdf5c8c72524041753`
...
### Historical M1.2 final readiness acceptance — EXECUTED / USER RESULT REJECTED
...
- The semantic-correction candidate at implementation commit
  `a884124718eab4bcdb0f98c59ae67acc7008f2fd`, ZIP SHA256
  `c2b91d30aade188a133626b578e976857a10abd57f3f24efd4100bab2820f293`,
  was the only installed Golden Key Skill for one WorkBuddy 5.3.14 / `Hy4 preview
  0.00x` ordinary request at 2026-08-30 19:50 local time.
- LauncherReceipt SHA256 is
  ...
  renderer, or production decision. The exact task directory
...
- The dialogue correctly exposed the FFmpeg baseline and four choices, but review
  returned `REJECT / P0=0 / P1=3 / P2=0`: partial capability groups were shown
  as ready, the first-use result expanded a broad Provider/configuration
  catalogue, and Remotion/HyperFrames were not separately explained across
  source integration, project dependencies, runtime readiness, and real
  invocation verification.
- Exact state at that checkpoint: `WORKBUDDY_FACT_RELAY_REACHED_DIALOGUE /
  FINAL_READINESS_USER_RESULT_REJECTED_P1_3 / NOT_PROVED / M1_3_BLOCKED`.

#### Historical authorized bounded correction — EXECUTED / SUPERSEDED

1. Update only the six authority/state documents, obtain one independent
   zero-write review, commit once, and ordinarily push
   `codex/workbuddy-capability-onboarding`.
2. Ordinarily merge that exact pushed head into the retained M1 branch, then
   change only `workbuddy-skill/golden-key-openmontage/SKILL.md`. Do not change
   Shell, Package, tests, or another Skill source.
3. The initial readiness summary is limited to FFmpeg, Remotion, HyperFrames,
   external video generation, and TTS. It must not expand raw setup offers,
   unrelated capability rows, a broad Provider catalogue, or a readiness
   dashboard.
4. “Ready” requires the necessary dependencies, a true runtime state, and real
   Package invocation evidence. Partial counts, adapter presence, or available
   Provider names cannot make a whole group ready. Remotion and HyperFrames each
   keep source integration, dependency, runtime, and invocation states separate.
5. Keep unconfigured enhancements configurable rather than product incapability,
   retain all four choices, and do not prescribe an exact transcript. Provider
   details remain for a relevant later configuration choice, not the first-use
   inventory.
6. Build one newly named uninstalled Skill ZIP bound to the active slim
   PackageRoot, obtain one independent zero-write candidate review, commit once,
   and ordinarily push the M1 branch. The currently installed same-name Skill
   must not be overwritten or automatically uninstalled.
7. Stop before WorkBuddy. No install, WorkBuddy run, optional capability,
...
That historical correction produced implementation commit
...
#### WorkBuddy-led optional configuration — HISTORICAL / EXECUTED THROUGH V8 BASE CONNECTION / NO CURRENT AUTHORITY
...
The historical implementation write set was:
...
That gate permitted focused local checks, one newly named Skill ZIP, review and
bounded closeout. It led to the v8 base-connection result and is closed. It gives
no current implementation, Skill/Remotion install, WorkBuddy, credential,
Provider, Package, or media authority. The 2026-09-04 current route amendment
governs all next action.
...
Owner direction dated 2026-08-29 corrects the earlier planning scope after
read-only official-document and Package inspection:
...
- M1.3 owns the selected mutation, consent, validation, recovery, and later
  ...
  and actual invocation. API-key completion requires secure non-chat input and
  storage, exact allowlisting, no disclosure, and an authorized connection test.
- Natural-language examples are design hypotheses. Real WorkBuddy behavior may
  use different wording, tools, or a bounded fixed choice/confirmation sequence.
...
Planning examples are not a required WorkBuddy transcript:
...
5. One representative local capability is installed, rediscovered, recognized,
   and actually invoked through the Package. One representative API-key Provider
   is securely configured outside ordinary chat and passes an authorized test
   without secret disclosure.
6. A later natural-language request containing `金钥匙智能体` can inspect,
   configure, change, or retest. A bounded guided sequence is allowed when real
   WorkBuddy or safety needs it; the user does not learn a command grammar.
7. No user-visible internal path, hash, schema, environment name, argv, pipe, or
...
result_4: COMPLETE
current_task: INSTALLER_PACKAGE_FLOW_VERIFIED / SKILL_LIFECYCLE_BLOCKED_EXTERNAL / M1_3_WAITING
...
current_implementation_worktree: HEAD_32EE9E03BAE2AF063D30D25781D8FF37621E09B6 / LOCAL_TRACKING_ADVERTISED_EQUAL / CLEAN
...
m1_3_result: V8_BASE_CONNECTION_ONLY / FORMAL_INSTALLER_RELEASE_ABSENT / REMOTION_REAL_SETUP_NOT_PROVED / API_ROUTE_REAL_AUTH_NOT_PROVED / M1_4_BLOCKED
...
installer_release_assembly: INDEPENDENT_MUST_PREREQUISITE / NESTED_RELEASE_FLOW_REJECTED / SINGLE_EXTRACTION_IMPLEMENTATION_PUSHED_32EE9E03BAE2AF063D30D25781D8FF37621E09B6 / FOCUSED_170_PASS_1_SKIP / REVIEW_APPROVE_P0_0_P1_0_P2_0 / CANDIDATE_SHA256_3B628B61AF50A075AF92CAD8B74A9FBB36D11380DE2AEAAB99ED25C3F22AECBB / LOCALAPPDATA_RESIDUE_REMOVED / PROGRAMFILES_CLEANUP_CANCELLED_BY_USER / PACKAGE_FLOW_VERIFIED / SKILL_LIFECYCLE_BLOCKED_EXTERNAL
