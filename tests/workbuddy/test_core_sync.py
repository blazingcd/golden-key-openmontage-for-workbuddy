...
def _release_config(
    zip_path: Path, lock_path: Path, zip_sha256: str, sidecar_name: str
) -> dict:
    lock = json.loads(lock_path.read_text(encoding="utf-8"))
    return {
        "golden_key_core_repository": "blazingcd/golden-key-openmontage",
        "golden_key_core_tag": "golden-key-v0.3.21",
        "golden_key_core_source_commit": lock["source_commit"],
        "golden_key_core_zip_asset": zip_path.name,
        "golden_key_core_zip_sha256": zip_sha256,
        "golden_key_core_sha256_asset": sidecar_name,
        "golden_key_core_lock_asset": lock_path.name,
        "golden_key_core_lock_sha256": _sha256(lock_path.read_bytes()),
        "golden_key_core_contract_id": lock["contract_id"],
        "golden_key_core_bundle_sha256": lock["bundle_sha256"],
        "upstream_base_commit": lock["upstream_base"],
        "authority": {
            "invocation_model": "direct_agent",
            "nested_agent_host_allowed": False,
            "consumer_direct_official_sync_allowed": False,
        ...
        "official_direct_sync_allowed": False,
        "private_git_ancestry_merge_allowed": False,
    }
...
def test_cached_release_assets_require_the_pinned_sha256_sidecar(tmp_path: Path):
    ...
    sidecar_name = "golden-key-v0.3.21-workbuddy-core.zip.sha256"
    (asset_dir / sidecar_name).write_text(
        f"{'0' * 64}  {zip_path.name}\n", encoding="utf-8"
    )
    config = _release_config(zip_path, lock_path, zip_sha256, sidecar_name)

    with pytest.raises(
        sync_workbuddy_core.SyncContractError, match="SHA-256 sidecar mismatch"
    ):
        sync_workbuddy_core.verify_release_asset_cache(asset_dir, config)


def test_sync_release_cli_reuses_verified_cache_and_is_idempotent(
    ...
    assert sync_workbuddy_core.main() == 0
    ...
    assert first["assets_reused"] is True
    assert first["changed_file_count"] == 2
    assert second["assets_reused"] is True
    assert second["changed_file_count"] == 0
    assert second["deleted_file_count"] == 0
    assert (destination / "requirements.txt").read_text(encoding="utf-8") == "consumer requirements\n"
    assert (destination / "setup.py").read_text(encoding="utf-8") == "consumer setup\n"
...
def test_release_cache_miss_downloads_only_the_three_pinned_assets(
    ...
    sidecar_name = "golden-key-v0.3.21-workbuddy-core.zip.sha256"
    (source_assets / sidecar_name).write_text(
        f"{zip_sha256}  {zip_path.name}\n", encoding="utf-8"
    )
    config = _release_config(zip_path, lock_path, zip_sha256, sidecar_name)
    calls: list[list[str]] = []
    ...
    asset_dir = tmp_path / "cache" / "golden-key-v0.3.21"

    bundle, report = sync_workbuddy_core.prepare_release_asset_cache(
        asset_dir, config, gh_executable="fake-gh"
    )

    assert len(bundle.entries) == 2
    assert report["assets_downloaded"] is True
    assert report["assets_reused"] is False
