...
Release-specific skill identity, release/authority owner, exact
PackageToolDefinition identity, interpreter, module, schema, fixed argv, and
hashes are supplied by the final Installer/Package gate. The adapter binds the
complete request definition to those stamped values before the single Stage 4
call. Unresolved placeholders, identity drift, guessed physical paths, or
missing provenance must fail closed and cannot be treated as a production
installation.
...
GOLDEN_KEY_WORKBUDDY_PACKAGE_TOOL_DEFINITION_ID=<installer:package_tool_definition_id>
GOLDEN_KEY_WORKBUDDY_PACKAGE_TOOL_DEFINITION_SHA256=<installer:package_tool_definition_sha256>
GOLDEN_KEY_WORKBUDDY_PACKAGE_TOOL_DEFINITION_RELATIVE_PATH=<installer:package_tool_definition_relative_path>
...
set is the seventeen fixed names above, plus the request's sorted
