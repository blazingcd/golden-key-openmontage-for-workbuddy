...
3. If `doctor.complete_production_environment.status` is not `ready`, invoke the launcher with `runtime plan --json`. Describe it in plain language as preparing the complete video production environment. The one confirmation covers the plan's pinned Python, FFmpeg, Node, Remotion, and HyperFrames components plus their managed browser; never infer this consent from the original video request.
4. Before asking, summarize the reported download size and registered `<data_root>/Runtime` destination, state that preparation does not modify system Python or the system PATH, disclose the FFmpeg GPLv3 terms, and point out that Remotion licensing is free only within the conditions stated by Remotion and may require a commercial licence for larger teams or certain automated use. Run `runtime prepare --confirm-download --json` only after the user explicitly agrees, then rerun `doctor --json`.
5. Do not ask the user to choose between Remotion and HyperFrames for installation. Both are standard local composition capabilities in this profile. Later, at the production proposal, follow `AGENT_GUIDE.md`: explain available render choices in plain language, recommend the one suited to the brief, and obtain the approval required by the Core decision contract.
6. If the user declines preparation, planning, onboarding, and API-key guidance may continue, but local media processing and final composition remain blocked. State that limitation; do not claim the production chain is ready and do not recommend reinstalling the ZIP for a runtime-only state.
7. Read `AGENT_GUIDE.md` before production from the reported `install_root` and obey Rule Zero.
8. Invoke the same launcher with `config inspect --json` (the registered form of `golden-key-workbuddy config inspect --json`). Keep the WorkBuddy conversation model separate from every Golden Key production Provider: WorkBuddy owns its conversation-model configuration, while the adapter only reports Tool Registry-backed production paths.
9. When a user wants a credential reference file, invoke the launcher with `config template` (the registered form of `golden-key-workbuddy config template`). It writes a consumer-owned template under the registered `data_root`, records environment-variable names only, and never stores credential values.
10. Report missing local runtimes and production Provider configuration without silently changing the requested production path. A China-ecosystem model reached through a third-party gateway is not a direct domestic Provider path.
11. Invoke `config guide --json` before proposing a generated-image, generated-video, TTS, avatar, or other API-backed production path. Start from its Chinese goal labels and recommend only the one or two Providers needed by the approved plan. For each, state direct-vendor or gateway status, friendly credential-field names, the `official account or key-management link`, account/permission caveat, billing notice, and current state; keep technical environment-variable names out of the normal conversation.
12. Do not ask the user to paste an API Key into WorkBuddy chat. Ask whether they want to configure now. After explicit opt-in, use `Start-Process` when local command execution is available to open the registered `配置API密钥.cmd` in a visible interactive window; otherwise tell them how to double-click it. Never put a Key in process arguments. Wait for the local hidden-input wizard to finish, then rerun `config guide --json`. Treat `present_unverified` as “已录入但未验证”, not as a successful connection.
13. Saving a Key does not authorize a Provider call, connectivity test, or paid generation. Obtain separate explicit approval before any network or Provider validation.
